...
trait TimeTrait
{
    ...
    public static function createFromTimestamp(int|float $timestamp, string $timezone = 'UTC', ?string $locale = null): self
    {
    // Use gmdate to format the timestamp in UTC and create a new instance of the object
    $time = new self(gmdate('Y-m-d H:i:s', (int) $timestamp), 'UTC', $locale);

    // Set the timezone if it's different from UTC
