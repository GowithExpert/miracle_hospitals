...
<head>
	<title>Manage Blood Account</title>
	...
	<div class="equl_mrgn">
		...
		<div>
			...
			<div class="card-content" id="brdr_botm_silvr">
				<h5 class="font_weght"><span class="fa fa-medkit col_blu"></span> Manage Blood Accountant Account</h5>
			</div>
			<div class="card-content" id="brdr_botm_silvr">
				<?php if ($blood_accounts) : ?>
					<!--Search Bar & Filter Bar Section Start -->
					<div class="row">
						<div class="col l8 m8 s12">
							<?= form_open('Admin/search_blood_account'); ?>
							<ul id="search_doctor">
								<li>
									<div class="input-container">
										<input type="text" name="search_med_name" class="serch_area" id="input_box" value="<?= set_value('search_med_name'); ?>" placeholder="Enter Accountant Name" required="">
						...
						<div class="col l2 m2 s2">
							<span class="right">
								<div class="tooltip"><a href="<?= base_url('Admin/blood_bank_accountant') ?>"><i class="fa fa-medkit med_icon" aria-hidden="true"></i></a>
									<span class="tooltiptext">Add Blood Bank Account</span>
								</div>
							</span>
						</div>
						<div class="col l2 m2 s10">
							...
							<ul class="dropdown-content" id="doctor_filter">
								...
								<li><a href="<?= base_url('Admin/filter_bld_account/new_med_acc'); ?>" class="waves-effect" id="dotted_border">
										<span class="fa fa-users col_blu"></span> New Accountant </a></li>
								<li><a href="<?= base_url('Admin/filter_bld_account/old_med_acc'); ?>" class="waves-effect">
										<span class="fa fa-users col_blu"></span> Old Accountant</a></li>
							</ul>
						</div>
			...
			<div class="scroll-container card-content">
				<table class="table">
					<tr class="backgrnd_colr_gray">
						...
						<th class="txt_align">Profile</th>
						<th class="txt_align">Status</th>
						<th class="txt_align_rgt">Action</th>
					</tr>
					<?php

					if (isset($blood_accounts)) :
						//count($blood_accounts);
						//echo '<pre>';
						//print_r($blood_accounts); exit;
						foreach ($blood_accounts as $doc) : ?>
							<tbody>
								<tr>
									<td class="text-container_tody_appoint">
										...
										<a class="tooltipped" data-position="top" data-tooltip="<?= $doc['username']; ?>">
											<?php
											if (isset($doc['med_image']) && !empty($doc['med_image'])) {
												if (file_exists(FCPATH . 'uploads/medicine_image/' .  $doc['med_image'])) { ?>
													<img src="<?= base_url() . 'public/uploads/medicine_image/' . $doc['med_image']; ?>" class="responsive-img" id="profile_pic" height="50">
									...
									<td class="txt_break-at300 txt_align">
										<span class="break-word">
											<a class="colour_hver" href="mailto:<?= $doc['email']; ?>"><?= $doc['email']; ?></a></span>
									</td>
									<td class="txt_break-at300 txt_align">
										<?php if ($doc['level'] == 2) :
											echo '<span class="col_gren">Accountant</span>';
										elseif ($doc['level'] == 5) :
											echo '<span class="col_gren">Front-desk/Doner</span>';
										else :
											echo '<span class="col_red">Un-Known</span>';
										?>
										<?php endif; ?>
									</td>
									<td class="txt_break-at300 txt_align">
									...
									<td class="text-container_tody_appoint txt_align_rgt">
										...
										<ul class="dropdown-content action_dropdown" id="action_dropdown_<?= $doc['id']; ?>" id="dotted_border">
											...
											<?php if ($doc['status'] == "Active") :  ?>
												<li><a href="<?= base_url('Admin/change_blood_acc_status/' . $doc['id'] . '/InActive'); ?>" id="dotted_border" style="color: red">
											...
											<?php elseif ($doc['status'] == "InActive") : ?>
												<li><a href="<?= base_url('Admin/change_blood_acc_status/' . $doc['id'] . '/Active'); ?>" id="dotted_border"><span class="fa fa-eye col_blu"></span> Active</a></li>
												<li><a href="javascript:void(0);" onclick="showDeleteConfirmationModal(<?= $doc['id']; ?>);" id="dotted_border" style="color:red;"><span class="fa fa-trash col_red"></span> Delete</a></li>

											<?php elseif ($doc['status'] == "Deleted") : ?>
												<li><a href="<?= base_url('Admin/change_blood_acc_status/' . $doc['id'] . '/Active'); ?>" id="dotted_border"><span class="fa fa-eye col_blu"></span> Active</a></li>
												<li><a href="<?= base_url('Admin/permanent_del_bld_acc_account/' . $doc['id']); ?>" id="dotted_border" style="color:red;"><span class="fa fa-trash col_red"></span> Permanent Delete</a></li>
											<?php else : ?>
												<li><a href="<?= base_url('Admin/change_blood_acc_status/' . $doc['id'] . '/Active'); ?>" id="dotted_border"><span class="fa fa-eye col_blu"></span> Active</a></li>
											<?php endif; ?>
										</ul>
						...
						<?php endforeach; ?>
					<?php else : ?>
						<h6 class="col_red">Medical Accountant Account Not Found</h6>
					<?php endif; ?>
					<tr>
						<td colspan="7">
		...
		<script>
			...
			function deleteItem(itemId) {
				window.location.href = "<?= base_url('Admin/delete_bld_acc_account'); ?>/" + itemId + "/Deleted";
			}
