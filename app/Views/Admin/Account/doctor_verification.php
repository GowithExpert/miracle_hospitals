	...
	<div class="equl_mrgn">
		...
		<div>
			...
			<div class="card-content" id="brdr_botm_silvr">
				<h5 class="font_weght"><span class="fa fa-user-md col_blu"></span> Verify Doctor Account</h5>
			</div>
			<div class="card-content" id="brdr_botm_silvr">
				<?php if ($doctors) : ?>
						...
						<div class="col l6 m6 s12">
							<span class="right">
								<button type="button" class="btn waves-effect waves-light dropdown-trigger btn_hver filter_btn" data-target="doctor_filter">
									<span class="fa fa-filter"></span> Filter Doctor Account
								</button>
							</span>
							<!---Student filter -->
							<ul class="dropdown-content" id="doctor_filter">

								<li><a href="<?= base_url('Admin/filter_doctor_verification/new_doc_account'); ?>" class="waves-effect" id="dotted_border">
										<span class="fa fa-users col_blu"></span> New Doctor Accountant </a></li>
								<li><a href="<?= base_url('Admin/filter_doctor_verification/old_doc_account'); ?>" class="waves-effect">
										<span class="fa fa-users col_blu"></span> Old Doctor Accountant</a></li>
							</ul>
						</div>
			...
			<div class="scroll-container card-content">
				<table class="table">
					...
					<?php if ($doctors) :
						...
						foreach ($doctors as $acc_account) : //	echo "<pre>"; print_r($acc_account);die;?>
							<tbody>
								<tr>
									<td>
										<center>
										<?php
										if (isset($acc_account['profile_pic']) && !empty($acc_account['profile_pic'])) {
											$imagePath = FCPATH . 'uploads/doctor/' . $acc_account['profile_pic'];
											if (file_exists($imagePath)) {
										?>
												<img src="<?= base_url() . 'public/uploads/doctor/' . $acc_account['profile_pic']; ?>" class="profile-photo" id="profile_pic" height="50">
											<?php } else { ?>
									...
									<td>
										...
										<ul class="dropdown-content action_dropdown" id="action_dropdown_<?= $acc_account['id']; ?>">
											...
											<?php if ($acc_account['status'] == "InActive") :  ?>
												<li><a href="<?= base_url('Admin/change_doc_acc_status/' . $acc_account['id'] . '/Active'); ?>" id="dotted_border">
														<span class="fa  fa-eye-slash col_gren"></span> 
														Verify</a></li>
											<?php else : ?>
												<li><a href="<?= base_url('Admin/change_doc_acc_status/' . $acc_account['id'] . '/Active'); ?>" id="dotted_border"><span class="fa fa-eye col_blu"></span> Active</a></li>
											<?php endif; ?>
										</ul>
										...
										<div id="deleteConfirmationModal" class="modal">
											<div class="modal-content">
												...
												<p>Are you sure you want to delete this Doctor Account?</p>
		...
		<script>
			...
			function deleteItem(itemId) {
				window.location.href = "<?= base_url('Admin/delete_doc_account'); ?>/" + itemId + "/Deleted";
			}
