...
<body>
    <script>
        ...
        function goBack() {
            ...
            window.location.href = '<?= base_url('Admin/doctors_available_slots/' . session()->get('patient_session_id'))  ?>';
        }
    ...
    <div class="container">
        <div class="card">
            <div class="appoint_inner">
                ...
                <!-- <//?= form_open("",array('id' => 'registration_form', 'onsubmit' => 'return updateCountryCode();')); ?> --> <!-- form Restrict submit page -->
                <div class="row">
                    <!-- **************search box code ***************** -->
                    <div class="topdv">
                        <div class="row">
                    ...
                    <div class="row">
                       <!-- Patient Unique ID - START  -->
                        ...
                        <div class="col-lg-6 col-md-6 col-sm-12">
                            ...
                            <?php } else { ?>
                                <input type="text" name="puid" id="puid" id="puid" value="" class="wp-form-control mrgn_botm wpcf7-text readonly_bg txt_fnt plcehld_clor" onblur="(this.type='text')" placeholder=" Search Patient ID Above (For Old Patients Only)" readonly autocomplete="off">
                            <?php } ?>
                        </div>
                    </div>
                        ...
                        <div class="col-lg-6 col-md-6 col-sm-12">
                            <div class="container1">
                                <div class="date-input" onclick="openModal();">
                                    <?php if (!isset($dt) || $dt == '') { ?>
                                        <input type="date" name="appointment_date" class="wp-form-control input_font_fam mrgn_botm wpcf7-text readonly_bg txt_fnt plcehld_clor indnt8" placeholder="Appointment Date" required="required" autocomplete="off">
                                    <?php } else { ?>
                    ...
                    <!-- <//?= form_close(); ?> --> <!-- form Restrict to submit page -->
                </div>
            </div>
        </div>
    </div>
    ...
    $(document).ready(function() {
        ...
        $("#btn_register_now").on("click", function (e) {
            ...
            if (valid) {
                ...
                $.ajax({
                    //type: "GET",
                    type: "POST",
                    url: "<?= base_url('Admin/book_appointment') ?>",
                    data: {
                    ...
                    success: function (response) {
                        if (response.status) {
                            var responseData = response.data;
                             var url = "<?= base_url('Admin/run_appointment_fee_form/') ?>"+
        ...
        $("#lookup").keyup(function () {
            ...
            if (srchval.length % 3 === 0 && srchval.length !== 0 && srchval.length !== lastSearchLength) {
                ...
                $.ajax({
                    url: "<?= base_url('/Admin/patients_search_lookup') ?>",
                    type: "POST",
                    dataType: "json",
