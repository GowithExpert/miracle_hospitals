...
<body>
	...
	<div id="preloader1"> <!-- id="preloader" is not allowing show php output, so renamed as preloader1 -->
        <div id="status" class="preloader_gif"></div>
    </div>
	<!---Body Section Start -->
	<div class="container">
		<div class="card">
			...
			<div class="card-content">
				<div class="row">
					<div class="col-lg-6 col-md-6 col-sm-12">
						...
						<?php //echo "<pre>"; print_r($departments); ?>
	...
	<script type="text/javascript">
		...
		$('#ward_name').change(function () {
			...
			console.log(ward_id);
			$.ajax({
				url: "<?= base_url('/Admin/get_beds_for_admission/') ?>" + ward_id,
				method: "POST",
				data: {
					ward_id: ward_id,
				},
		...
		$("#btn_register_now").on("click", function () {
			...
			$.ajax({ //Ajax call - START
				type: 'POST',
				url: "<?= base_url('/Admin/admission_process/') ?>",
				data: {
				...
				if (responseData.message) {
					...
					window.location.href = "<?= base_url('/Admin/manage_patients') ?>?success=" + encodedsucesMsg;
				} else {
					// Encode the error message and redirect to another page
					var encodedErrorMessage = encodeURIComponent(responseData.error);
					window.location.href = "<?= base_url('/Admin/manage_patients') ?>?error=" + encodedErrorMessage;
				}
			},
