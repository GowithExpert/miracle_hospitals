    ...
    <div id="multi-step-form-container">
        ...
        $(document).ready(function() {
            ...
            $('#goto_step2_btn').click(function(event) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "<?= base_url('/Doctor/upload_revisit_prescription/') ?>",
                    data: {
            ...
            //     $.ajax({
            //         type: "POST",
            //         url: "<//?= base_url('/Admin/upload_prescription_report/' . $res->id); ?>/" + id + '/' + browse_rpt_id + '/' + prescription_id,
            //         data: formData,
            //         contentType: false,
            //         processData: false,
            //         success: function(response) {
            ...
            $(".file-input").change(function() {
                ...
                $.ajax({
                    type: "POST",
                    url: "<?= base_url('/Doctor/upload_rev_prescription_report/' . $res->id); ?>/" + id + '/' + browse_rpt_id + '/' + prescription_id,
                    data: formData,
                    contentType: false,
                    processData: false,
                    success: function(response) {
            ...
            $('#goto_step4_btn').click(function(event) {
                ...
                if ($.trim(advice) != '') {
                    $.ajax({
                        type: 'POST',
                        url: "<?= base_url('/Doctor/save_revisit_pat_advice') ?>",
                        data: data,
                        dataType: 'text', // for 'string'
            ...
            $('#save').click(function(event) {
              ...
              var slct_refr_usr = $('#selctarea').find('option:selected').text();
                ...
                $.ajax({
                    type: 'POST',
                    url: "<?= base_url('/Doctor/save_revisit_pat_message/') ?>",
                    data:{
                    ...
                    success: function(response) {
                        try {
                            ...
                            if (jsonResponse.message) {
                                ...
                                window.location.href = "<?= base_url('/Doctor/manage_revisit_patient')?>?sucesMsg=" + encodedMessage;
                            } else {
        ...
        $("#goto_step3_btn").on("click", function() {
            ...
            if (checked.length != 0) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "<?= base_url('/Doctor/add_rev_prescription_report/' . $res->id); ?>",
                    data: dataArr,
                    dataType: 'text', //for 'string'
                    success: function(response) { // Handle the success response
                        var jsonResponse = $.parseJSON(response);
