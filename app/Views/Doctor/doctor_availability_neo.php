...
<script>
    $(document).ready(function() {
        ...
        var jsonResponse = ''; //Addressing noticesCALENDAR DAY AJAX CALL
        var currentDateYYYYMMDD; //Initialize currentDateYYYYMMDD 
        var currentDate  = new Date();
        var currentYear  = currentDate.getFullYear();
        var currentMonth = currentDate.getMonth() + 1;//Month is at 0 index
        var currentDay = currentDate.getDate();

        currentDateYYYYMMDD  = currentYear + '-' + ("0" + currentMonth).slice(-2) + '-' + ("0" + currentDay).slice(-2);
        
        
        populateSlotGrid(); // function call- to population slot grid
        ...
        function populateSlotGrid() {
            ...
            if(!Array.isArray(availableSlots)) {
                ...
                availableSlots.forEach(function(slot) {
                    ...
                    if(slotnum === 72){ order = 1; } //(1st slot is 00:00--00:15. 72th slot is 23:45--00:00. 72th to 96th slots are NEXT DATE slots.
        ...
        */
        ...
        function getSelectedDate() {
            const selectedDay = $('#calendarDays .calendar-day.selected .day-number').text();
            const selectedMonth = $('#monthHeader').text();
            const selectedYear = $('#yearHeader').text();
            const months = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];

            if (typeof currentDateYYYYMMDD !== 'undefined' && currentDateYYYYMMDD !== null) {
                if (!selectedDay || !selectedMonth || !selectedYear) {
                    selectedDate = currentDateYYYYMMDD;
                } 
                else {
                    const monthIndex = months.indexOf(selectedMonth);
                    if (monthIndex !== -1 && typeof selectedDay !== 'undefined' && selectedDay !== null  && typeof selectedYear !== 'undefined' && selectedYear !== null ) { //month array `index` does not match 
                        selectedDate = `${selectedYear}-${String(monthIndex + 1).padStart(2, '0')}-${String(selectedDay).padStart(2, '0')}`;
                    } 
                    else {
                        selectedDate = currentDateYYYYMMDD; // fallback if month is invalid
                    }
                }
            }
            console.log("The selectedDate is: "+selectedDate);
            return (typeof selectedDate !== 'undefined' && selectedDate !== null) 
                ? selectedDate 
                : currentDateYYYYMMDD;
        } //function - Closed
       ...
       /* @param:
        * @desc: On Day/date click: for getting doctor available 
        * slots on that date
        * @use:
        * @date: August, 2029, 2023
        * @author: Nearks Team
       ...
       /* @param:
        * @desc: On Selection: Ajax call to Save selected doctor 
        * vailablity slot
        * @use:
    ...
    */
    ...
    function isPastSlot(slot) {
        const todayDate = getTodayDateYyyyMmDd(); //return current date
        const selectedDate = getSelectedDate();    // Get selected date
        
        // Use Intl.DateTimeFormat instead of locale string parsing
        const currentTime = new Date();
        const indiaTime = new Intl.DateTimeFormat("<?= LOCALE_TM_STR ?>", {
            timeZone: "<?= APP_TIMEZONE ?>",   // From Constants.php
            hour: "<?= HRS_DIGITS ?>",         // Usually "2-digit"
            minute: "<?= MIN_DIGITS ?>",       // Usually "2-digit"
            hour12: false
        }).format(currentTime); // e.g., "16:30"

        // Ensure numeric values
        const [cu_hours, cu_minutes] = indiaTime.split(':').map(Number); 

        // Extract the start time from the slot
        const slotStart = slot.slice(1).split('--')[0]; // safely get "HH:mm"
        const [st_hours, st_minutes] = slotStart.split(':').map(Number);

        // Check only if selected date is today
        if (todayDate === selectedDate) { //selectedDate 
            if (st_hours < cu_hours || (st_hours === cu_hours && st_minutes <= cu_minutes)) {
                return true; // Slot is in the past or current
            }
        }
        return false; // Slot is in the future or date is not today
    } //function - Closed
