...
<body>
			...
			<div class="card-content">
				...
				<div class="row">
					<div class="col-lg-6 col-md-6 col-sm-12">
						...
						<div class="select-wrapper">
							...
							<select name="department_name" id="department_name" class="asterisk departmentSelect input_box" >
					...
					<div class="col-lg-6 col-md-6 col-sm-12">
						...
						<div class="select-wrapper">
							...
							<select name="ward_name" id="ward_name" class="asterisk wardSelect input_box">
				...
				<div class="row">
					<div class="col-lg-6 col-md-6 col-sm-12">
						...
						<div class="select-wrapper">
							<select name="bed_lable" id="bed_lable" class="asterisk bedSelect input_box">
					...
					<h6><span class="fa fa-tasks" style="color: #005197"></span> Other Info</h6>
						<!-- <input type="text" placeholder="Enter the other Info" id="input_box"> -->
						<textarea name="other_info" class="asterisk" placeholder="Enter Other Information"id="input_box"></textarea>
					</div>
				</div>
					<div class="row row12">
						<div class="col">
							<button type="submit" id="btn_register_now" class="btn btn-waves-effect waves-light btn_hver sub_btn" style="text-transform: capitalize;font-weight: 500;font-size: 16px;background: #005197"><span class="fa fa-user"></span>  Addmission Process</button>
						</div>
					</div>		
				<!-- <//?= form_close(); ?> -->
			</div>
	...
	<script type="text/javascript">
		...
		$('#department_name').change(function() {
			...
			//console.log(dept_id);
			$.ajax({
				url: "<?= base_url('/Frontdesk/get_wards_for_admission/') ?>"+dept_id,
				method: "POST",
				data: {
					dept_id: dept_id,
				},
	...
	$('#ward_name').change(function() {
		$("#preloader1").show();
			...
			console.log(ward_id);
			$.ajax({
				url: "<?= base_url('/Frontdesk/get_beds_for_admission/') ?>"+ward_id,
				method: "POST",
				data: {
					ward_id: ward_id,
				},
		...
		$("#btn_register_now").on("click", function () {
			...
			$.ajax({ //Ajax call - START
				type: 'POST',
				url: "<?= base_url('/Frontdesk/admission_process/') ?>",
				data: {
				...
				success: function (response) {
					...
					if (responseData.message) {
						// Set success message for redirection page
						var sucesMsg = responseData.message;
						window.location.href = "<?= base_url('/Frontdesk/manage_patients?success=') ?>" + encodeURIComponent(sucesMsg);
					} else {
						// Set error message for redirection page
						var errorMessage = jsonResponse.error || 'Something went wrong. Please try again.';
						window.location.href = "<?= base_url('/Frontdesk/manage_patients?error=') ?>" + encodeURIComponent(errorMessage);
					}
				},
				error: function (xhr, textStatus, errorThrown) {
					...
					// Check if the response is JSON and contains a message property
					if (xhr.responseJSON && xhr.responseJSON.message) {
						// Set error message for redirection page
						var errorMessage = xhr.responseJSON.message;
						window.location.href = "<?= base_url('/Frontdesk/manage_patients?error=') ?>" + encodeURIComponent(errorMessage);
					} else {
						// Set a generic error message for redirection page
