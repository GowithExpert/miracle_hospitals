...
<body> <!---Body Section Start --->
    ...
    <?php //echo "<pre>"; var_dump($dr_slots); die; ?>
    <?php 
    if($dr_slots) {
    if(!empty($dr_slots)) {
        //echo "<pre>";print_r($groupedSlots);die;
        foreach($dr_slots as $docarr) { $doc = (object) $docarr; ?>
            <div class="row <?php echo (empty($doctors[$doc->id])) ? 'brdr_bottom' : 'dr_slots_avail'; ?>" id="slotGridRow<?php echo $doc->id;?>">
            ...
            <form id="slotfrm<?php echo $doc->id; ?>" class="brder_bottm" name="slotfrm" action="" method="post">
                <div class="slot-grid mrgn_lft_rght">
                    ...
                    <?php  foreach($doc->slots as $slotsarr){ $slots = (object) $slotsarr; ?> 
                        
                        <div id="shwgrid" onclick="toggleSlotSelectionNew($(this),'<?php echo $slots->slot_id;?>','<?php echo $slots->doctor_id;?>');" class="slot"><?php echo $slots->start_end_slot; ?> <span hidden=""><?php echo $slots->doctor_id; ?></span></div>
    ...
    <script>
                            ...
                            if (!jsonResponse.status) { 
                                ...
                                if(slotData && slotData != 'undefined') {
                                    ...
                                    if (Array.isArray(slotData) && slotData.length > 0) { 
                                        ...
                                        $('#dr_div_ajax').text(slot.doctor_name);
                    ...
                    availableSlots.forEach(function (slot) {
                       // console.log(slot);
                        ...
                        slotElement.on("click", function () {
                            toggleSlotSelectionNew($(this), slot.id, slot.doctor_id);
                        });
                    });
                    $('#slotGridAjax').show();
                    $('#rspns_errtxt_msg').hide();
         ...
         * @param: 
         */ 
        function toggleSlotSelectionNew(slotElement, slotid, drid) {
