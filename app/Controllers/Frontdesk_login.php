...
class Frontdesk_login extends BaseController {
	...
	// }
	public function index() { 
		return view('Frontdesk/frontdesk_account'); 
	}
	public function create_frnd_account(){
		$data  = [];
		...
		if ($this->request->getMethod() == 'post') {
			$rules = [
				...
				'mobile'    =>	[
					'rules'      => 'required|numeric|exact_length[10]|is_unique[blood_bank_users.mobile]',
					'errors'     => [
				...
				$uid = md5(str_shuffle('abcdefghizklmnopqrstuvwxyz'.time()));
					...
					if ($insert_data) {
						...
						$message   = 'Hi ' .$this->request->getVar('username',FILTER_SANITIZE_STRING).",<br><br>Thanks Your Account Created Successfully, Please Click the below Link to Activate your Account <br><br>"
						   ."<a href='".base_url()."/Frontdesk_login/Activate_account/".$uid."' target='_blank'>Activate  Now</a><br><br>Thanks<br> " 
						   . DEV_TEAM; 
		...
		return  view('Frontdesk/frontdesk_account', $data);
	}
	...
	public function forget_password(){
		...
		if ($this->request->getMethod() == 'post') {
			...
			if ($this->validate($rules)) {
				...
				$this->is_unique = $this->commonForAllModel->is_single_record('blood_bank_users', 'email', $this->email_to);
				...
				$this->save_status = $this->registerModel->update_rec_by_args('blood_bank_users', $this->updt_args, $this->updt_data_arr );
				if ($this->save_status) {
					...
					$message  = 'Hi,'
								...
								.'<a href="'.base_url().'/Frontdesk_login/reset_password/'.$this->token.'">Click Here to Reset Password</a>'
								.'<br>Thanks <br> '
								. DEV_TEAM . '<br>'
	...
	*/
	public function reset_password($token = null) {
		...
		$data = [];
		if (!empty($this->token)) { 
			$userdata = $this->commonForAllModel->get_records_for_token('blood_bank_users', $this->token);
			if (!empty($userdata)) {
				...
				if ($check_exp_time) { 
					if ($this->request->getMethod() == 'post') { 
						...
						if ($this->validate($rules)) {
							...
							$update_pass = $this->registerModel->update_rec_by_args('blood_bank_users', $this->updt_args, $this->updt_data_arr );
							if ($update_pass) {
						...
						return view('Frontdesk/reset_acc_password', $data);
					}
				}
				else {
	...
	public function Activate_account($unid = null){
		...
		return view('Frontdesk/Activate_acc_account', $data);
	}
	...
	public function login_account() { //site login 
		$this->data = [];
		// Site login
		if ($this->request->getMethod() == 'post') {
			$rules = [
			...
			if ($this->validate($rules)) {
				...
				//$userroles = []
				$userroles = array(0, 1, 2, 3, 4); //0: Frontdesk (& Donor), 1: Admin 2: Accountants Role, 3: 
	
				if ($allow) {
					$this->data['userdata'] = $this->doctorModel->verifyAccountantEmail('blood_bank_users', $email);
					...
					if (!isset($this->data['userdata']['password']) || $this->data['userdata']['password'] == '') {
						...
						if ($this->data['userdata']['status'] == 'InActive') {
							...
							//var_dump($this->data['userdata']['level']);die;
							if ($this->data['userdata']['level'] === '5') {
								...
								if ($login_activity_id) { $this->session->set('loggedin_info', $login_activity_id); }

								$this->session->set('frontdesk_session_uid', $this->data['userdata']['uid']);
								$this->session->set('frontdesk_session_id', $this->data['userdata']['id']);
								$this->session->set('frontdesk_session_name', $this->data['userdata']['username']);
								$this->session->set('frontdesk_session_pic', $this->data['userdata']['profile_pic']);
				...
				else { $this->data['error'] = 'Max No. of failed Login Attempt, Try Again a Few Minutes'; }
			} 
			else {
		...
		return view('Frontdesk/frontdesk_login', $this->data);
	} //function - Closed
